...
int key_uncrypt_Bcd(unsigned char* input,unsigned char* key)
{
	...
	_encrypt_(bits,DESCRYPT);		/* descrypt */
...
void pan_encrypt_Bcd(unsigned char* input, unsigned char* pan,unsigned char* key)
{
	...
	_encrypt_(bits,ENCRYPT);		/* encrypt */
...
{
    unsigned char left_key[9] = {0};
	unsigned char mid_key[9] = {0};
    unsigned char right_key[9] = {0};
    unsigned char tmpstr1[9] = {0}, tmpstr2[9] = {0};
    
    memcpy( left_key, doublekey, 8 );
	memcpy( mid_key, doublekey + 8, 8 );
    memcpy( right_key, doublekey+16, 8 );
    
    Des( input, left_key, tmpstr1 );
    Undes(tmpstr1, mid_key, tmpstr2 );
    Des( tmpstr2, right_key, output );
}
...
void TriUnDes(unsigned char *input, unsigned char *doublekey, unsigned char *output)
{
    ...
    Undes( input, right_key, tmpstr1 );
    Des( tmpstr1, mid_key, tmpstr2 );
    Undes( tmpstr2, left_key, output );
}

void pcyan3DESEncode(const unsigned char *input, const unsigned int inputLen, const unsigned char *doublekey, unsigned char *output, unsigned int* pDestLen)
{
	// 
    unsigned int count = 0;
	unsigned char partChar[8+1] = {0};
	int copyLen = 0;
	unsigned char paddinglen = 0;
	int i = 0;
	char dest8[8+1] = {0};
	unsigned int destLen = (inputLen+7)/8*8;
	if (inputLen % 8 == 0)	// paddings
		destLen += 8;
    
	*pDestLen = destLen;
	memset(output, 0, destLen + 1);

    while (1) {
		memset(partChar, 0, 8+1);

        copyLen = inputLen - count;
        if (copyLen > 8) {
            copyLen = 8;
        }
        memcpy(partChar, input + count, copyLen);
        paddinglen = 8 - copyLen;

		// 
        for (i = copyLen; i < 8; i ++) {
            partChar[i] = paddinglen;
        }
        
        memset(dest8, 0, 8+1);
        TriDes((unsigned char*)partChar, (unsigned char*)doublekey, (unsigned char*)dest8);
        memcpy(output + count, dest8, 8);
...
void pcyan3DESDecode(const unsigned char *input, const unsigned int inputLen, const unsigned char *doublekey, unsigned char *output, unsigned int* pDestLen)
{
	//  
    unsigned int count = 0;
	unsigned char partChar[8+1] = {0};
	...
	*pDestLen = destLen;
    ...
    while (1) {
        ...
        memset(dest8, 0, 8+1);
        TriUnDes((unsigned char*)partChar, (unsigned char*)doublekey, (unsigned char*)dest8);
        memcpy(output + count, dest8, 8);
	...
	// 
	paddingChar = output[destLen - 1];
	if (paddingChar <= 8)
	{
		for (i = 0 ; i < paddingChar ; i ++)
		{
			// 
			if (output[destLen - i - 1] != paddingChar)
			{
				isPadding = 0;
				break;
			}
