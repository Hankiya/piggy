...
public class ShareHelper {
	private static String WXAPPID = "wx76e03ddac369bcf3";// 微信开发平台注册应用的AppID
	...
	public static void showSharePicker(Context cx, DialogInterface.OnClickListener l) {
		...
		if (mDlgShare == null || mDlgShare != null) {
			View view = LayoutInflater.from(cx).inflate(R.layout.com_share_pop, null);
			...
			mDlgShare = new AlertDialog.Builder(cx).setView(view).create();//new ContextThemeWrapper(cx,R.style.dialog)
	...
	private static void shareWx(Context cx, boolean circle, String title,String content, String contentUrl,
			SnsPostListener l,UMImage umImage) {
	...
	/*
	 * public static List getShareApps(Context context) { List mApps = new
	 * ArrayList(); Intent intent = new Intent(Intent.ACTION_SEND, null);
	 * intent.addCategory(Intent.CATEGORY_DEFAULT); intent.setType("audio/*");
	 * PackageManager pManager = context.getPackageManager(); mApps =
