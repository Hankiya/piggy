...
public class FieldVerifyCard {
	...
	private static final int[] SORTCODES2 = new int[] { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2 };
	...
	private static String checkArea(String idCardInput) {
		...
		if (areaCode != HONGKONG_AREACODE && areaCode != TAIWAN_AREACODE && areaCode != MACAO_AREACODE && (areaCode > MAX_MAINLAND_AREACODE || areaCode < MIN_MAINLAND_AREACODE))
	...
	/**
	 * http://sarin.iteye.com/blog/597306
	 * @deprecated
	 * @param idno17
	 * @return
	 */
	private static String checkCode(String idno17) {
	...
	private static String checkCheckCode(int idCardType, String idCard) {
		if (idCardType == EIGHTEEN_IDCARD) {
			...
			for (int i = 0; i < chars.length-1; i++) {//
				sum=sum+(Integer.parseInt(String.valueOf(chars[i])) * SORTCODES2[i]);
			}
