...
public class BezierParam {
	...
	public BezierParam(final Path path, float smoonth) {
		mBPath = path;
		setSmoonth(smoonth);
	}
	 ...
	 * @throws
	 */
	public Path getLastPath(boolean interupt) {
		float x = mCur.x + (mCur.x - mPre.x) / mInitAdjust;
		float y = mCur.y + mCur.y - mPre.y;
		return getNextPath(x, y, interupt);
	}
	 ...
	 * @throws
	 */
	public Path getNextPath(float nextX, float nextY, boolean interupt) {

		if (nextX != mCur.x || nextY != mCur.y) {
			mNxt.set(nextX, nextY);
			mMidNx.set(mNxt.x - mCur.x, mNxt.y - mCur.y);
			mLenNx = (float) Math.sqrt(mMidNx.x * mMidNx.x + mMidNx.y * mMidNx.y);
			mMidNx.set((mCur.x + mNxt.x) / 2, (mCur.y + mNxt.y) / 2);
		}
		return calCtrlAdPath(interupt);
	}
	 ...
	 * @throws
	 */
	public void setFirstPoint(float curX, float curY, float nxtX, float nxtY) {
		mCur.set(curX, curY);
		mPre.set(curX + (curX - nxtX) / mInitAdjust, curY + curY - nxtY);
		mMidPr.set(mPre.x - mCur.x, mPre.y - mCur.y);
		mLenPr = (float) Math.sqrt(mMidPr.x * mMidPr.x + mMidPr.y * mMidPr.y);
	...
	private Path calCtrlAdPath(boolean interupt) {
		if (mLenNx != 0) {
			calCtrlPoint();
			return getBezier3Path(interupt);
		}
		return mBPath;
	}
	...
	private Path getBezier3Path(boolean interupt) {// 更新曲线路径.
		if (interupt) {
			mBPath.reset();
			mBPath.moveTo(mPre.x, mPre.y);
		}
		mBPath.cubicTo(mCtrlRect.left, mCtrlRect.top, mCtrlRect.right, mCtrlRect.bottom, mCur.x,
				mCur.y);
		reset();
		return mBPath;
	}
	...
	private Path getBezier2Path(boolean interupt) {// 更新曲线路径.
		if (interupt) {
			mBPath.reset();
			mBPath.moveTo(mPre.x, mPre.y);
		}
