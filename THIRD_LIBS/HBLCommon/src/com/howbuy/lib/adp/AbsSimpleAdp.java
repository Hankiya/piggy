...
public abstract class AbsSimpleAdp<T> {
	...
	public AbsSimpleAdp(Context cx) {
		this(LayoutInflater.from(cx), null);
	}

	public AbsSimpleAdp(LayoutInflater lf) {
		this(lf, null);
	}

	public AbsSimpleAdp(Context cx, ArrayList<T> items) {
		this(LayoutInflater.from(cx), items);
	}

	public AbsSimpleAdp(LayoutInflater lf, ArrayList<T> items) {
		this.mLf = lf;
	...
	public final View getView(int pos, View view, ViewGroup p) {
		...
		if (view == null) {
			...
			view.setTag(holder);
			holder.initHolderData(pos, type, mItems.get(pos), false);
		} else {
			holder=(AbsViewHolder<T>) view.getTag();
			holder.initHolderData(pos, type,
					mItems.get(pos), true);
		}
		return view;
	}

	public void recycleView(int pos, View v) {
	}
