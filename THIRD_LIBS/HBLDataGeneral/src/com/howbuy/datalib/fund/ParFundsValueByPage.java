...
import com.howbuy.wireless.entity.protobuf.FundInfoClosesNewOfPageProto.FundInfoClosesNewOfPage;
import com.howbuy.wireless.entity.protobuf.FundInfoMoneysOfPageProto.FundInfoMoneysOfPage;
import com.howbuy.wireless.entity.protobuf.FundInfoOpensOfPageProto.FundInfoOpensOfPage;
/**
 * 
 * type:o代表开放式、7代表货币式、a代表封闭式、53代表理财类、1：股票型、3：混合型、5：债券型、8：指数型、t：结构型、9：QDII基金、b：保本型(理财类基金返回的字段跟货币式一样)
orderType：排序类型：
 ...
 */
public class ParFundsValueByPage extends AbsParam{
	public static final String URL="start/fund/navupdatepage.protobuf";
	//type	int	N	基金分类(*)
	private String type	=null;//	N	基金分类(*)
	...
	public ParFundsValueByPage setParams( String type, int cstmFlag, String cstmCodes,
	...
	public ParFundsValueByPage(long cacheTime) {
		super(cacheTime);
	}
	public ParFundsValueByPage(int handType, Handler uiHand, long cacheTime) {
		super(handType, uiHand, cacheTime);
	}

	public ParFundsValueByPage(int handType, IReqNetFinished calbk, long cacheTime) {
		super(handType, calbk, cacheTime);
	}

	public ParFundsValueByPage(String key, int handType, IReqNetFinished calbk,
			Handler uiHand, long cacheTime) {
		super(key, handType, calbk, uiHand, cacheTime);
	}

	public ParFundsValueByPage(String key, IReqNetFinished calbk, long cacheTime) {
		super(key, calbk, cacheTime);
	}
	@Override
	public Object parseToObj(InputStream is, ReqNetOpt opt) throws Exception {
	   if("7".equals(type)||"53".equals(type)){
		 return  FundInfoMoneysOfPage.parseFrom(is);
	   }else if("a".equals(type)){
		   return  FundInfoClosesNewOfPage.parseFrom(is);
	   } else{
		   try{
			   return  FundInfoOpensOfPage.parseFrom(is);
		   }catch(Exception ex){
			   ex.printStackTrace();
		   }
		  return null;
