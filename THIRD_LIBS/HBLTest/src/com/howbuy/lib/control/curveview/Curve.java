...
public class Curve {
	...
	public Curve(CtrlOption ctrlOpt, RectF rect,
	...
	public float getYAt(ICharData data) {
		float result = -1;
		if (data != null) {
			result = mRect.top - mScale
					* (data.getValue(mCtrlOpt.mDataType) - mMaxCoord);
		}
	...
	protected int[] adjustStartAdEnd() {
		...
		se[1] = Math.min(mCtrlOpt.mCapacity + mCtrlOpt.mIOffset + 2,
				mLen);
		return se;
	}
	...
	public void whenDataChanged(boolean calScale, boolean calcuVisible) {
		if (mLen > 0) {
			...
			if (calcuVisible) {
				...
				end = Math.min(mCtrlOpt.mIOffset + mCtrlOpt.mCapacity
						+ 1, mLen);
			}
