...
public class CurveControl {
	...
	protected boolean mEnablePadRight = false;
	 ...
	 * @throws
	 */
	protected void setCurveList(
			HashMap<ICharType, ArrayList<? extends ICharData>> datas) {

		Iterator<Entry<ICharType, ArrayList<? extends ICharData>>> list = datas
				.entrySet().iterator();
		while (list.hasNext()) {
			Entry<ICharType, ArrayList<? extends ICharData>> data = list.next();
			mCurveList.put(data.getKey(),
					createCurve(data.getValue(), data.getKey()));
		}
	}

	protected Curve createCurve(ArrayList<? extends ICharData> datas,
			ICharType charType) {
		Curve curve = null;
		if (mFactory != null) {
			curve = mFactory.createCurve(mCtrlOpt, new RectF(mRecCurve), datas,
					charType);
		} else {
			curve = CurveFactory.getDefCurve(mCtrlOpt, new RectF(mRecCurve),
					datas, charType);
		}
		int size = curve.size();
		mSize += 1;
	 ...
	 *            ignore it.
	 */
	public boolean addCurve(ICharType charType,
			ArrayList<? extends ICharData> data, boolean measure) {
	...
	public boolean addCurveData(Curve curve,
			ArrayList<? extends ICharData> datas, boolean end) {
	...
	protected ArrayList<CharDataType> getValuesAt(int index) {
		mSelectResult.clear();
		Iterator<Entry<ICharType, Curve>> list = mCurveList.entrySet()
				.iterator();
		while (list.hasNext()) {
	...
	protected void whenFramChanged() {
		...
		if (!mCtrlOpt.mHasInitWithData) {
			mCtrlOpt.initSpace(mEnablePadRight,
					mCurve == null ? 0 : mCurve.size(),
	...
	protected void adjustMinMax(boolean checkYmin) {
		if (mSetting.mCoordUpWeight != 0 || mSetting.mCoordBotWeight != 0) {
			float lenY = (mMax1 - mMin1)
					/ (1 - mSetting.mCoordBotWeight - mSetting.mCoordUpWeight);
			mMin = mMin1 - lenY * mSetting.mCoordBotWeight;
			mMax = mMax1 + lenY * mSetting.mCoordUpWeight;

			if (checkYmin && mMin < 0) {
				mMax = mMax1 + mSetting.mCoordUpWeight * (mMin1)
						/ mSetting.mCoordBotWeight;
	...
	protected void drawRect(Canvas can, Paint paint, int type, int startColor,
			int endColor, float dval) {
		mPath.reset();
		if (type == 0) {
		...
		mCtrlOpt.drawPathShade(can, mPath, mRect,
				new Rect(0, 0, (int) mRect.width(), (int) mRect.height()),
				startColor, endColor);
	}

	protected void drawBackgroud(Canvas can, Paint paint) {
		if (mSetting.mEnable3D) {
			int startColor = mSetting.mBackYColor, endColor = ViewUtils.color(
					startColor, 0.5f, 1);
			mRect.set(mRecCurve);
			...
			can.drawLine(mRecCurve.left, mRecCurve.bottom, mRecCurve.left,
					mRecCurve.top, paint);
			can.drawLine(mRecCurve.left, mRecCurve.bottom, mRecCurve.right,
					mRecCurve.bottom, paint);
			can.drawLine(mRecCurve.right, mRecCurve.top, mRecCurve.right,
					mRecCurve.bottom, paint);
			can.drawLine(mRecCurve.right, mRecCurve.top, mRecCurve.left,
					mRecCurve.top, paint);
		}
	...
	protected void drawGridColAdXText(Canvas canvas, Paint paint,
		...
		while (x <= recCurve.right) {
			...
			if (mSetting.mEnableCoordTxtX) {
				...
				ICharData data = mCtrlOpt.mMaxVisibleLen != 0 ? mCurve
						.getDataAt(id).mData : null;
				...
				if (i++ == mSetting.mCol) {
					xOffset = Math.min(0, (mRecFrame.width() - recCurve.right)
							- paint.measureText(xStr) / 2);
				}
				paint.setPathEffect(null);
	...
	protected void drawCoord(Canvas canvas, Paint paint) {
		...
		RectF recCurve = mRecCurve;
		float top = recCurve.top - mSetting.mArrowSize;
		float right = recCurve.right + mSetting.mArrowSize;
		float arowLen = 6;
		float arowspace = arowLen / 2;
		if (mSetting.mEnableCoordY) {
			canvas.drawLine(recCurve.left, recCurve.bottom, recCurve.left, top,
					paint);
			canvas.drawLine(recCurve.left, top, recCurve.left - arowspace, top
					+ arowLen, paint);
			canvas.drawLine(recCurve.left, top, recCurve.left + arowspace, top
					+ arowLen, paint);
		}
		if (mSetting.mEnableCoordX) {
			canvas.drawLine(recCurve.left, recCurve.bottom, right,
					recCurve.bottom, paint);
			canvas.drawLine(right, recCurve.bottom, right - arowLen,
					recCurve.bottom - arowspace, paint);
			canvas.drawLine(right, recCurve.bottom, right - arowLen,
					recCurve.bottom + arowspace, paint);
		}
	}
	...
	protected void drawYText(Canvas canvas, Paint paint) {
		...
		RectF recCurve = mRecCurve;
		float txtCenterVerticalOffset = ViewUtils
				.getTxtCenterVerticalOffset(mSetting.mCoordTxtYSize);
		float txtLeft = recCurve.left;
		if (mSetting.mEnableTxtYInSide) {
			paint.setTextAlign(Align.LEFT);
			txtLeft -= txtCenterVerticalOffset;
		} else {
		...
		paint.setStrokeWidth(mSetting.mDensity);
		float mDRow = mRecCurve.height() / mSetting.mRow;
		float y = recCurve.top + txtCenterVerticalOffset;
	...
	public int pointToIndex(float x, float y, boolean pointClick) {
		...
		if (mCtrlOpt.mMaxVisibleLen > 0) {
			index = Math.min(
					Math.round((mRecCurve.right - mCtrlOpt.mOffsetRx - x)
							/ (mCtrlOpt.mSpace)), mCtrlOpt.mCapacity - 1);
			index = Math.max(
					Math.min(mCtrlOpt.mMaxVisibleLen - 1, index
							+ mCtrlOpt.mIOffset), 0);
			if (pointClick) {
	...
	protected void setupCanvas() {
		...
		if (mCanvasBmp != null)
			mCanvasBmp.recycle();
		try {
			mCanvasBmp = Bitmap.createBitmap((int) mRecFrame.width(),
					(int) mRecFrame.height(), Config.ARGB_8888);
	...
	public final class CtrlOption {
		...
		int checkMoveAble(boolean right, boolean adjust) {
			int result = 0;
			if (right) {
				float maxOffset = (mMaxVisibleLen - 1) * mSpace - mRect.width()
						+ mSpace / 2;
		...
		protected void initSpace(boolean mEnablePadRight, boolean showAllData) {
			...
			if (mEnablePadRight) {
				...
				if (!mHasInitWithData) {
					if (showAllData) {
						if (mMaxVisibleLen > 0) {
							mSpace = mRect.width() / (mMaxVisibleLen + 0.5f);
							mSpace = Math.max(
									Math.min(mSetting.MAX_SPACE, mSpace),
									mSetting.MIN_SPACE);
					...
					if (mSetting.mEnableInitCent && initSpace < mRect.width()) {
						mOffsetX = ((mMaxVisibleLen - 1) * mSpace - mRect
								.width()) / 2;
						mIOffset = (int) (mOffsetX / mSpace);
		...
		boolean adjustSpace(boolean mEnablePadRight, float newSpace) {
			...
			newSpace = Math.max(mSetting.MIN_SPACE,
					Math.min(newSpace, mSetting.MAX_SPACE));
		...
		public boolean drawCoordTxt(Canvas g, ICharType type, ICharData data,
				RectF rect, boolean isX) {
			if (!mSetting.mEnableCoordTxtX) {
				Paint paint = mPaint;
				paint.setTextSize(mSetting.mCoordTxtXSize);
				paint.setColor(mSetting.mCoordTxtXColor);
				float offset = ViewUtils.getTxtHeight(mSetting.mCoordTxtXSize)
						/ 2
						- ViewUtils
								.getTxtCenterVerticalOffset(mSetting.mCoordTxtXSize)
						+ 3;
				g.drawText(data.getTime("MM/dd"), rect.centerX(), mRect.bottom
						+ offset + mSetting.mCoordSize, paint);
			}
			return !mSetting.mEnableCoordTxtX;
		}

		public void drawPathShade(Canvas g, Path path, RectF recLinear,
				Rect rectBounds, int startColor, int endColor) {
			ShapeDrawable shaper = new ShapeDrawable(new PathShape(path,
					recLinear.width(), recLinear.height()));
			LinearGradient linear = new LinearGradient(recLinear.left,
					recLinear.top, recLinear.right, recLinear.bottom,
