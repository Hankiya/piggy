...
public class BezierShadeLine extends Curve{
	...
	public BezierShadeLine(CtrlOption ctrlOpt, RectF rect,
	...
	protected void drawCurve(Canvas g, Paint paint, float dx, float padR) {
		...
		if (mLen > 1) {
			...
			for (int i = se[0]+1; i < se[1]; i++, x -= dx) {
				...
				paint.setShader(new LinearGradient(mBezier.getPrePoint().x, mBezier.getPrePoint().y, mBezier.getCurPoint().x, mBezier.getCurPoint().y, mData.get(i-2).getColor(false), mData.get(i-1).getColor(false), TileMode.CLAMP));
			...
			paint.setShader(new LinearGradient(mBezier.getPrePoint().x, mBezier.getPrePoint().y, mBezier.getCurPoint().x, mBezier.getCurPoint().y, mData.get(se[1]-2).getColor(false), mData.get(se[1]-1).getColor(false), TileMode.CLAMP));
			g.drawPath(path, paint); 
			path.lineTo(mBezier.getCurPoint().x, mRect.bottom);
			path.lineTo(mBezier.getPrePoint().x, mRect.bottom);
			path.close();
