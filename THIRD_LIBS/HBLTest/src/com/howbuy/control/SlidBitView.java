...
import com.howbuy.lib.compont.RateHelper;
...
public class SlidBitView extends View implements IAnimChaged {
	...
	public SlidBitView(Context context, AttributeSet attrs) {
		...
		mRateHelper = new RateHelper(new Handler(), null, this);
	}

	public void setInterpolator(Interpolator l) {
		mRateHelper.setInterpolation(l);
	}
	...
	public boolean setCurrentNum(float num, boolean ignoreSame) {
		if (mCurValue != num || ignoreSame) {
			...
			if (mBitDuration > 0 && isCanvasVisible()) {
				...
				d("setCurrentNum", "duration=" + (mBitDuration * mMaxBit) + ",bitDuration="
						+ mBitDuration + ",period=" + period + " ,maxbit=" + mMaxBit + ",num="
						+ num);
	...
	private void drawUnSlidChar(Canvas can, float x, float y) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (isPoint) {
				...
				can.drawText(mDesChars, i, 1, mRecTemp.left + mPointPad - mDensity, y, mPainBk);
			} else {
