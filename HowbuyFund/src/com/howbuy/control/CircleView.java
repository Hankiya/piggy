...
public class CircleView extends AbsView {
	...
	public static int FLAG_ENABLE_FIRST_ANIM = 512;
	...
	public void setEnableFirstAnim(boolean enable) {
		if (enable) {
			addFlag(FLAG_ENABLE_FIRST_ANIM);
		} else {
			subFlag(FLAG_ENABLE_FIRST_ANIM);
		}
	}

	public boolean isEnableFirstAnim() {
		return hasFlag(FLAG_ENABLE_FIRST_ANIM);
	}
	...
	public CircleView(Context arg0, AttributeSet arg1) {
		super(arg0, arg1);
		setFlag(FLAG_ANIM_ALPHA | FLAG_ANIM_SCALE | FLAG_ANIM_SWEEP | FLAG_ANIM_ROATE
				| FLAG_ANIM_EXTEND_SELECTED | FLAG_ENABLE_INDICATOR | FLAG_ENABLE_SELECT
	...
	private void drawPies(Canvas can, RectF recOut, RectF rcIn, float offset) {
		...
		for (int i = 0; i < mLen; i++) {
			...
			sDegree += data.mWhat;
			if (mDivider > 0 && mLen > 1) {
				mPaint.setColor(!mAnimStop && hasFlag(FLAG_ANIM_ALPHA) ? color(mDividerColor)
						: mDividerColor);
	...
	private void drawArcCenTex(Canvas can, RectF rcIn, String txt) {
		...
		can.drawText(txt, rcIn.centerX(),
				rcIn.centerY() - ViewUtils.getTxtCenterVerticalOffset(txtSize), mPaint);
	}
	...
	public boolean onViewFirstSteped(int w, int h) {
		if (hasFlag(FLAG_ENABLE_FIRST_ANIM)) {
	...
	public boolean animToIndex(int index, boolean setSelected) {
		...
		if (index < 0 || index >= mLen
				|| (hasFlag(FLAG_ENABLE_KEEPSELECTED) && mPreSelected == index)
				|| (!hasFlag(FLAG_ENABLE_KEEPSELECTED) && mPreSelected == index)) {
			...
			if (hasFlag(FLAG_ANIM_EXTEND_SELECTED)) {
				...
				addFlag(FLAG_ENABLE_CENTXT | FLAG_ENABLE_INDICATOR | FLAG_ENABLE_SELECT
						| FLAG_ENABLE_KEEPSELECTED);
			} else {
			...
			int duration = (int) Math.min(
					(1 - mRoateRate) * Math.max(Math.abs(mRoateCycle) * 12, 200), 1800);
			if (duration == 0) {
				mOffset += mRoateCycle;
				mOffset %= 360;
				invalidate();
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_UP: {
			if (mDownSelected != -1) {
				...
				if (select != -1) {
					if (mListen != null) {
						mListen.onSelectedChaged(select, mPreSelected,
								hasFlag(FLAG_ENABLE_INDICATOR));
					}
